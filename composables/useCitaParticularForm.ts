// composables/useCitaParticularForm.ts
import { ref, computed } from "vue";
import { citasParticularService } from "~/services/CitasParticularService";
...
export function useCitaParticularForm() {
  ...
  function obtenerDiasSemana(): Array<{
    fecha: string;
    dia: string;
    numero: number;
    activo: boolean;
  ...
  async function buscarServiciosDisponibles() {
    ...
    try {
      // Usar el servicio específico para citas particulares
      serviciosDisponibles.value = await citasParticularService.getServiciosDisponibles(
        fechaCita.value
      );
  ...
  async function buscarMedicosDisponibles() {
    ...
    try {
      // Usar el servicio regular para médicos
  ...
  async function buscarHorariosDisponibles() {
    ...
    try {
      // Usar el servicio regular para horarios
  ...
  async function registrarCitaParticular() {
    ...
    try {
      // Usar el servicio específico para registrar citas particulares
      const resultado = await citasParticularService.registrarCita(dataCita);
      return resultado;
    } catch (error) {
      console.error("Error:", error);
      throw error;
